...
- PR gate: `.github/workflows/pr-inner-loop.yml`
- Nightly/manual deep audit: `.github/workflows/nightly-deep-audit.yml`
...
- What changed: connected `parameter-golf` workflows to local `sentinelayer-v1-action`, enabled model-training context, and added nightly static challenge validation.
...
- What still needs testing: first live PR run with Omar artifacts upload and one full candidate evidence cycle (`results/latest_candidate.json` + `results/history.csv` updated from a real training run).
